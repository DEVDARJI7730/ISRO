...
export const LoginPage: React.FC = () => {
  ...
  // Listen for Google Auth callback URL params
  useEffect(() => {
    ...
    if (token && userParam) {
      try {
        ...
        showToast('Google authentication successful! Initialising session.', 'success');
        // Force full page reload to let AuthContext initialize with the stored tokens
        window.location.href = '/dashboard';
      } catch (err) {
        console.error('Failed to parse Google user payload:', err);
        showToast('Google session validation failed.', 'error');
      }
    }
    ...
    <div className="relative min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
      ...
      <div className="max-w-md w-full glass-card p-8 md:p-10 relative z-10">
        <div className="text-center mb-8">
          <h2 className="font-display font-black text-3xl text-white mb-2">Systems Sign In</h2>
          <p className="text-sm text-slate-400">ISRO Image Colorization & Enhancement Portal</p>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 text-slate-700 font-semibold px-4 py-3.5 rounded-xl border border-slate-300 transition-all duration-200"
        >
        ...
        <div className="text-center mt-6 pt-6 border-t border-white/5 text-sm text-slate-400">
          New researcher?{' '}
